...
\begin{tikzpicture}[tdplot_main_coords]
  ...
  \draw[thin,red,dashed]  (3,0,0) -- (3,2,0) node[font=\small,pos=0.5,anchor=north] {$y$};
  \draw[thin,red,dashed]  (0,2,0) -- (3,2,0) node[font=\small,pos=0.25,anchor=north west] {$x$};
...
\begin{tikzpicture}[y=0.7cm, x=0.7cm,font=\sffamily]
  % ticks
  \draw[step = 2, gray, very thin,opacity=0.25] (-8.5, -8.5) grid ( 8.5, 8.5);
  % axis
  \draw[thick,->] (-8.5,0) -- coordinate (x axis mid) (8.5,0) node[anchor = north west] {$\hat{\imath}$};
  \draw[thick,->] (0,-8.5) -- coordinate (y axis mid) (0,8.5) node[anchor = south east] {$\hat{\jmath}$};
...
\begin{problem}
  ...
  \item A turtle starts at the origin. It moves at a constant speed of 2
    ...
    \begin{subproblem}
      ...
      \item What is the turtle's position after one minute?
        \sideNote{Determine the anti-derivative of the velocity and be sure to solve for the two constants.}
        \vfill
        \vfill
        \vfill
      \item What is the turtle's position at any time?
        \vfill
    \end{subproblem}
    \vfill
  ...
  \item An object starts at the point $3\hat{\imath} - 2\hat{\jmath}$, and it moves with a velocity of $-\hat{\imath}+4\hat{\jmath}$. Make a sketch of the $x-y$ plane that indicates all of the points the object moves through.
    \vfill

  \clearpage

\item Suppose that $\vec{r}(t)=\vec{r}_0+t\vec{v}_0$ is a line in space, and $P$ is a point in space.
  ...
  \begin{subproblem}
    ...
    \item Mark an arbitrary point on the line, call it $S$. Add a sketch of the direction of $\vec{v}_0$ at $S$.
    ...
    \item Which part of the triangle represents the distance between the point $P$ and the line? Determine a formular for the distance in terms of $\theta$ and $\overrightarrow{SP}$.
      \vfill
...
\item Suppose that $P$ is a point on a plane, and $\vec{n}$ is a vector that is perpendicular to the plane.
  ...
  \begin{subproblem}
    ...
    \item What is the angle between the vector $\overrightarrow{PQ}$ and the vector $\vec{n}$?
      \vspace{3em}
    \item What can you say about the dot product as well as the cross product of the vectors $\overrightarrow{PQ}$ and $\vec{n}$? Which is an easier relationship to work with?
      \label{activity:planes:normalRelationship}
      \vfill

      \clearpage

    \item Assume that $P$ is a fixed point on the plane, $P=x_0\hat{\imath}+y_0\hat{\jmath}+z_0\hat{k}$, and $Q$ is any point on the plane, $Q=x\hat{\imath}+y\hat{\jmath}+z\hat{k}$.
      Determine the vector $\overrightarrow{PQ}$.
        \vfill
    ...
    \sideNote{For a point on the plane just assume the values of some of the coordinates and ask what that implies about the other parts.}
       \vfill
     ...
     \begin{eqnarray*}
       2x - 4y + z & = & 1.
     \end{eqnarray*}
     Determine the vector that is normal to the plane. Determine a point in the plane, $P$. Then find two other points, $Q$ and $R$ so that $\vec{PQ}$ and $\vec{PR}$ are perpindicular to one another.
        \vfill

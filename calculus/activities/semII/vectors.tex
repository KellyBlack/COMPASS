...
\item Suppose $\vec{r}(t)$ is a line in space, and $P$ is a point in space.
  ...
  \item Determine the distance between the point $4\hat{\imath} + 3 \hat{\jmath} + \hat{k}$ and the line $\vec{r}(t)=\left(\hat{\imath}-2\hat{\jmath}-2\hat{k}\right)+t\left(\vec{\imath}+6\vec{\jmath}+6\vec{k}\right)$.
    \vfill
...
\item Suppose that $P$ is a point on a plane, and $\vec{n}$ is a vector that is perpendicular to the plane at the point $P$.
  ...
  \begin{subproblem}
    ...
    \item Assume that $P$ is a fixed point, $P=x_0\vec{\imath}+y_0\vec{\jmath}+z_0\vec{k}$, and $Q$ is any point on the plane, $Q=x\vec{\imath}+y\vec{\jmath}+z\vec{k}$.

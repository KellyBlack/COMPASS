...
\begin{problem}
\item A long, thin rod is five meters long and has a charge density given by
  \begin{eqnarray*}
    \lambda_q & = & 5x~\frac{\mathrm{C}}{\mathrm{m}},
  \end{eqnarray*}
  where $x$ is between 0$m$ and 5$m$.
  \begin{subproblem}
...
\item Determine the area of the slice of a sector shown below in grey in terms of
  $\theta$, $L_1$, and $L_2$.
...
Other graphs can be rotated around the axis, and the question is how
to find the resulting surface area given a formula for the graph.  The
basic idea to find the surface area of such a surface is to divide up
the domain of the function small pieces, revolve each little piece of the graph around
...
\begin{tikzpicture}[scale=1.5]
 ...
 \end{tikzpicture}
 \end{center}
\caption{The graph is divided up into pieces and each piece is
  revolved around the $x$-axis.}
\label{flux:figure:Riemann}
\end{figure}


For the general case, a graph of the function $y=f(x)$\ is rotated around the
$x$-axis from $x=a$\ to $x=b$, and the resulting surface area will be
found.  The first step is to break up the graph into small pieces. In
...
\begin{center}
 ...
 \begin{center}
   \begin{tikzpicture}[scale=1.5,>={Latex[angle=45:5pt 2]}]
     \fill[left color=gray!70,right color=gray!50,middle color=gray!02,shading=axis,shading angle=0,fill opacity=0.05] (2.7,1.5) -- (3.3,1.5) arc (90:270:0.2 and 1.5) -- (2.7,-1.5) arc (270:90:0.2 and 1.5);
     \draw[black] (2.7,1.5) -- (3.3,1.5) arc (90:270:0.2 and 1.5) -- (2.7,-1.5) arc (270:90:0.2 and 1.5);
     \fill[left color=gray!10!gray!30,right color=gray!10!gray!30,middle color=gray!5,shading=axis,shading angle=0] (2.7,1.5) -- (3.3,1.5) arc (90:-90:0.2 and 1.5) -- (2.7,-1.5) arc (-90:90:0.2 and 1.5);
     \draw[thick,->] (-0.1,0) -- (4,0) node[anchor=west] {$x$};
     \draw[thick,->] (0,-2) -- (0,2) node[anchor=south] {$y$};
     \fill[black] (0,0) circle (0.1);
     \draw[densely dotted,black] (0,0) -- (3, 1.5) -- (3,0);
...
\item Express each function below as a single fraction. That is bring
  ...
  \item ${\displaystyle \frac{1}{x+2} + \frac{3}{(x+2)^2}}$
    \vfill
  \item ${\displaystyle \frac{1}{x+3} + \frac{2x}{(x+3)^2}}$
    \vfill

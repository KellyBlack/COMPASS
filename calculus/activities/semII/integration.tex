...
\begin{problem}
\item Determine formulas for each of the following functions only in
  terms of $\sin(\theta)$ and $\cos(\theta)$. (Hint: These are
  trigonometric identities that you should be able to look up.)
  \begin{subproblem}
  \item $\cos(2\theta)=$
    \vfill
  \item $\sin(2\theta)=$
...
\item A rod has a charge distribution, and the length of the rod is
  ...
  charge densities. 
  \begin{subproblem}
    \item $\lambda_q =  2+\sin^2(\theta)\cos(\theta)$ C/m
      \vfill
    \item $\lambda_q = 2+\sin^2(\theta)\cos^3(\theta)$ C/m
      \vfill
  \end{subproblem}
\end{problem}
